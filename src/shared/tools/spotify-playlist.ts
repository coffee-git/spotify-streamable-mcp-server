...
export const spotifyPlaylistTool = defineTool({
  ...
  annotations: { title: toolsMetadata.spotify_playlist.title, readOnlyHint: false, openWorldHint: true },
  handler: async (args: SpotifyPlaylistInput, context: ToolContext): Promise<ToolResult> => {
    try {
      ...
      if (!args.playlist_id) return fail(args.action, 'playlist_id is required');
      ...
      if (args.action === 'items') {
        ...
        const items = (page.items ?? []).flatMap((entry, index) => {
          ...
          return raw ? [{ ...toSlimTrack(TrackCodec.parse(raw)), position: base + index }] : [];
        });
        return ok(args.action, { playlist_id: id, playlist_uri: `spotify:playlist:${id}`, limit: page.limit, offset: base, total: page.total, items }, `Loaded ${items.length} playlist item(s).`);
      }
      ...
      if (args.action === 'add_items') {
        if (!args.uris?.length) return fail(args.action, 'uris are required');
      ...
      if (args.action === 'remove_items') {
        ...
        if (!items?.length) return fail(args.action, 'items are required');
      ...
      if (args.range_start == null || args.insert_before == null) return fail(args.action, 'range_start and insert_before are required');
